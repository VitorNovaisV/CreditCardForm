...
//Selection of the Logo and Expiration Date Select elements
...
//Add the currentYear And The next 10 years as options to the "Expiration Date" Element

//Keys Functions manually redone due to multiple inputs acting like one in the "Card Number" Element(s)
...
document.addEventListener("keydown", (e) => {
  ...
  switch (key) {
    ...
    //ArrowLeft and ArrowRight actions that functions as a way do walk between inputs
    case "Delete": {
      if (
        input.selectionStart === input.value.length &&
        input.selectionEnd === input.value.length
    ...
    //Delete and Backspace actions with compatibility to copy and paste actions
    default: {
    ...
    //RegEx validation that only accepts numbers
  }
});
...
//RegEx validation that only accepts numbers, and paste the content in the correct input field
...
//Dynamic Logo Image Based in the 4 firsts digits
...
//Substitute digits based in the selection, without losing data
...
//automatically change the focus on multiple inputs while filling in the "Card Number" Element(s)
...
//Checks if the input in use is connected to others inputs, acting as one.
